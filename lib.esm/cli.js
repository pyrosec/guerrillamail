...
export async function saveSession(guerrillamail, json = false, filename = "session.json") {
    await mkdirp(path.join(process.env.HOME, ".guerrillamail"));
    await fs.writeFile(path.join(process.env.HOME, ".guerrillamail", filename), guerrillamail.toJSON());
    if (!json)
        logger.info("saved to ~/" + path.join(".guerrillamail", filename));
}
...
export async function loadSession() {
    const proxyOptions = await loadProxy();
    const guerrillamail = GuerrillaMail.fromJSON(await fs.readFile(path.join(process.env.HOME, ".guerrillamail", "session.json")));
...
export async function loadProxy() {
    await mkdirp(path.join(process.env.HOME, ".guerrillamail"));
    try {
        return proxyStringToObject(await fs.readFile(path.join(process.env.HOME, ".guerrillamail", "proxy"), "utf8"));
    }
    catch (e) {
        return null;
    }
...
export async function loadSessionFrom(name) {
    const guerrillamail = GuerrillaMail.fromObject(JSON.parse(await fs.readFile(path.join(process.env.HOME, ".guerrillamail", name), "utf8")));
    await saveSession(guerrillamail);
}
